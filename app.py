...
def calculer_forecast_simulation_v3(df, all_semaine_columns, selected_months, sim_type, progression_pct=0, objectif_montant=0):
    try:
        ...
        if sim_type=='Simple Progression':
            ...
            total_fcst_qty_period=total_n1_sales_selected_months_series*prog_factor
        ...
        tot_adj_qty_all_m=pd.Series(0.0,index=df_sim.index)
        tot_fin_amt_all_m=pd.Series(0.0,index=df_sim.index)
        for m_name in selected_months:
...
def render_supplier_checkboxes(tab_key_prefix,all_suppliers,default_select_all=False):
    sel_all_k=f"{tab_key_prefix}_select_all"
    sup_cb_ks={s:f"{tab_key_prefix}_cb_{sanitize_supplier_key(s)}"for s in all_suppliers}
    ...
    def toggle_all_s_tab():
        curr_sel_all_v=st.session_state[sel_all_k]
        for cb_k in sup_cb_ks.values():st.session_state[cb_k]=curr_sel_all_v
    def check_ind_s_tab():
        all_ind_chk=all(st.session_state.get(cb_k,False)for cb_k in sup_cb_ks.values())
    ...
    with st.expander(exp_lbl,expanded=True):
        ...
        st.markdown("---")
        sel_sups_ui=[]
        num_disp_cols=4;chk_cols=st.columns(num_disp_cols);curr_col_idx=0
...
if uploaded_file and st.session_state.df_full is None:
    ...
    dyn_key_prefs=['tab1_','tab2_','tab3_','tab4_','tab5_']
    ...
    try:
        ...
        excel_buf.seek(0)
        df_min_c_t=safe_read_excel(excel_buf,sheet_name="Minimum de commande")
        min_o_dict_t={}
        if df_min_c_t is not None:
            ...
            if s_c in df_min_c_t.columns and m_c in df_min_c_t.columns:
                try:
                    ...
                    st.success(f"✅ 'Min cmd' lu ({len(min_o_dict_t)}).")
        ...
        excel_buf.seek(0)
        df_suivi_t=safe_read_excel(excel_buf,sheet_name="Suivi commandes",header=4)
        if df_suivi_t is not None:
            ...
            if not miss_s_cols_c:
                ...
                st.success(f"✅ 'Suivi cmds' lu ({len(df_suivi_t)}).")
            else:
            ...
            st.info("Onglet 'Suivi cmds' non trouvé/vide. Suivi non dispo.")
            st.session_state.df_suivi_commandes=pd.DataFrame()
        ...
        st.error(f"❌ Err majeure chargement/traitement: {e_load_main}")
    ...
    with tab5: # Suivi Commandes Fournisseurs
        ...
        if df_suivi_cmds_all is None or df_suivi_cmds_all.empty:
            ...
            if not sups_in_suivi_list:st.info("Aucun fournisseur trouvé dans données suivi.")
                ...
                sel_f_t5_ui=render_supplier_checkboxes("tab5",sups_in_suivi_list,default_select_all=False)
                            ...
                            try:
                                with zipfile.ZipFile(zip_buf,'w',zipfile.ZIP_DEFLATED)as zipf:
                                    for sup_nm_s_exp in sel_f_t5_ui:
                                        ...
                                        excel_b=excel_buf_ind.getvalue()
                                        file_nm_in_zip=f"Suivi_Commande_{sanitize_sheet_name(sup_nm_s_exp)}_{pd.Timestamp.now():%Y%m%d}.xlsx"
                                        zipf.writestr(file_nm_in_zip,excel_b)
                                        files_added_zip+=1
                                if files_added_zip>0:
                                    zip_buf.seek(0)
                                    archive_nm=f"Archive_Suivi_Commandes_{pd.Timestamp.now():%Y%m%d_%H%M}.zip"
