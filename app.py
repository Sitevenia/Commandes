...
def calculer_quantite_a_commander(df, semaine_columns, montant_minimum_input, duree_semaines):
    ...
    # --- (Code inchangé par rapport à la version fonctionnelle précédente) ---
    try:
...
if uploaded_file and st.session_state.df_full is None:
    ...
    dynamic_keys = [k for k in st.session_state if k.startswith(('tab1_', 'tab2_'))] # Clear keys for tabs 1 & 2
    ...
    try:
        ...
        try:
            ...
            if len(df.columns) > start_col:
                ...
                # Identifier les colonnes semaine (format YYYYWW ou numérique)
...
if 'df_initial_filtered' in st.session_state and st.session_state.df_initial_filtered is not None:
    ...
    # --- Tabs ---
    tab1, tab2, tab3 = st.tabs(["Prévision Commande", "Analyse Rotation Stock", "Vérification Stock"]) # Reverted to 3 tabs
        ...
        if not selected_fournisseurs_tab1: st.info("Sélectionnez fournisseur(s) ci-dessus.")
            ...
            if st.button("🚀 Calculer Quantités", key="calc_cmd_btn"):
                ...
                if result_cmd:
                    ...
                    # Utiliser les noms de colonnes renvoyés par la fonction de calcul
                    df_res_cmd["Qte Cmdée"] = q_calc
                    df_res_cmd["Vts N-1 Total (calc)"] = vN1_tot
                    df_res_cmd["Vts 12 N-1 Sim (calc)"] = v12N1_sim
            ...
            if 'calc_res_df' in st.session_state and st.session_state.calc_res_df is not None:
                if st.session_state.sel_fourn_calc_cmd == selected_fournisseurs_tab1:
                    ...
                    # CORRECTED Indentation & Logic for Min Warning
                    ...
                    # Display Table using result column names
...
# --- App footer/initial message ---
    ...
    if st.button("🔄 Réinitialiser l'application"):
         ...
         dynamic_keys = [k for k in st.session_state if k.startswith(('tab1_', 'tab2_'))] # Removed tab4_
         keys_to_clear.extend(dynamic_keys)
         for key in keys_to_clear:
             if key in st.session_state: del st.session_state[key]
