...
if uploaded_file and st.session_state.df_full is None:
    # ... (début du bloc try...except inchangé) ...
    try:
        # ... (lecture excel et initialisation inchangées) ...
        st.session_state.min_order_dict = min_dict_temp # Affectation après le bloc try/except pour min order
        ...
        try:
            ...
            # --- Identification Colonnes Semaine ---
            start_col_index = 12 # Colonne N
            semaine_cols_temp = []
            if len(df.columns) > start_col_index:
                ...
                # Priorité au format YYYYWW
                semaine_cols_temp = [
                    c for c in pot_w_cols if c not in exclude
                    and isinstance(c, str) and len(c)>=6 and c[:4].isdigit() and c[4:6].isdigit()
                ]
                if semaine_cols_temp:
                    logging.info(f"Colonnes semaine identifiées par format YYYYWW: {len(semaine_cols_temp)}")
                else:
                    # Fallback vers colonnes numériques si YYYYWW non trouvé
                    logging.warning("Format YYYYWW non détecté, fallback vers colonnes numériques après index 12.")
                    semaine_cols_temp = [
                        c for c in pot_w_cols if c not in exclude
                        and pd.api.types.is_numeric_dtype(df.get(c, pd.Series(dtype=float)).dtype)
                    ]
                    if semaine_cols_temp:
                         logging.info(f"Colonnes semaine identifiées par type numérique: {len(semaine_cols_temp)}")

            # Trier les colonnes trouvées
            try:
                # Essayer de trier numériquement si possible (pour YYYYWW)
                st.session_state.semaine_columns = sorted(semaine_cols_temp, key=lambda x: int(x) if isinstance(x, str) and x.isdigit() else float('inf'))
            except ValueError:
                 # Trier alphabétiquement si le tri numérique échoue
                 st.session_state.semaine_columns = sorted(semaine_cols_temp)

            if not st.session_state.semaine_columns: # Vérification finale
                 logging.error("ERREUR CRITIQUE: Aucune colonne de vente hebdomadaire valide n'a pu être identifiée.")
                 st.error("❌ Impossible d'identifier les colonnes de ventes hebdomadaires après la colonne M. Vérifiez le format du fichier ou les noms des colonnes.")
                 # On pourrait stopper ici si les semaines sont essentielles pour TOUTE l'appli
                 # st.stop()
            else:
                 logging.info(f"Final semaine_columns count: {len(st.session_state.semaine_columns)}")
            ...
            for col in ess_num_cols:
                 ...
                 elif not df_init_filtered.empty: st.error(f"Col essentielle '{col}' manquante."); missing_essential = True
            if missing_essential: st.stop()
            st.rerun() # Rerun pour appliquer état de session
        # ... (except blocks inchangés) ...
    # ... (except block inchangé) ...

# --- Main App UI ---
# ... (Reste du code principal inchangé, il utilisera st.session_state.semaine_columns qui est maintenant déterminé de manière plus robuste) ...
...
if 'df_initial_filtered' in st.session_state and st.session_state.df_initial_filtered is not None:
    ...
    semaine_columns = st.session_state.semaine_columns # Utilisation de la variable d'état
    ...
    with tab1:
        ...
        # Vérification SI semaine_columns existe AVANT de proposer le calcul
        if not semaine_columns:
             st.warning("Impossible de calculer: Aucune colonne de ventes valide n'a été identifiée dans le fichier.")
        elif not selected_fournisseurs_tab1: st.info("Sélectionnez fournisseur(s) ci-dessus.")
        elif df_display_tab1.empty: st.warning("Aucun article trouvé.")
        else:
            # ... (Reste du code de Tab 1 inchangé) ...
    ...
    with tab2:
        ...
        elif not semaine_columns: st.warning("Colonnes ventes manquantes.") # Check if weeks exist
        else:
