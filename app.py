    ...
    try:
        ...
        if montant_minimum_input > 0 and mt_avant < montant_minimum_input:
            ...
            while mt_actuel < montant_minimum_input and iters < max_iter:
                ...
                idx_ptr -= 1
                idx_ptr += 1
            if iters >= max_iter and mt_actuel < montant_minimum_input:
                st.error("Ajustement montant min échoué (max iter).")
        mt_final = sum(q * p for q, p in zip(qcomm, tarif))
        return (qcomm, ventes_N1, v12N1, v12last, mt_final)
    except Exception as e:
        st.error(f"Erreur calcul qté: {e}")
...
def calculer_forecast_simulation(df, all_semaine_columns, selected_months, sim_type, progression_pct=0, objectif_montant=0):
    try:
        ...
        if len(all_semaine_columns) < 104:
            st.error("Données historiques insuffisantes (< 104 semaines) pour N-1.")
            return None
        ...
        if not all(col in df.columns for col in n1_week_cols):
            st.error("Erreur interne: Colonnes N-1 manquantes.")
            return None
        ...
        total_cols = ["Ventes N-1 Total (Mois Sélectionnés)", "Qté Totale Prév. (Mois Sel.)", "Montant Total Prév. (€) (Mois Sel.)"]
...
if uploaded_file and st.session_state.df_full is None:
    ...
    keys_to_clear_on_new_file = ['df_full', 'df_initial_filtered', 'semaine_columns', 'calculation_result_df', 'rotation_result_df', 'forecast_result_df', 'supplier_multiselect_key']
    ...
    try:
        ...
        if df_full_temp is None:
            st.error("❌ Échec lecture 'Tableau final'.")
            st.stop()
        ...
        if missing_on_load:
            st.error(f"❌ Colonnes manquantes: {', '.join(missing_on_load)}")
            st.stop()
...
if 'df_initial_filtered' in st.session_state and st.session_state.df_initial_filtered is not None:
        ...
        if not current_selection and fournisseurs_list:
            ...
            if st.button("🚀 Calculer Quantités", key="calc_cmd_btn"):
                with st.spinner("Calcul..."):
                    result_cmd = calculer_quantite_a_commander(df_display_filtered, semaine_columns, montant_minimum_input_cmd, duree_semaines_cmd)
                if result_cmd:
                    st.success("✅ Calcul OK.")
                    (q_calc, vN1, v12N1, v12l, mt_calc) = result_cmd
                    df_res_cmd = df_display_filtered.copy()
        ...
        if not current_selection and fournisseurs_list:
            ...
            if st.button("🔄 Analyser Rotation", key="analyze_rot_btn"):
                 with st.spinner("Analyse..."):
                    df_rot_res = calculer_rotation_stock(df_display_filtered, semaine_columns, sel_p_w)
    ...
    with tab3:
        st.header("Vérification Stocks Négatifs")
    ...
    with tab4:
        ...
        if df_display_filtered.empty:
             if current_selection:
                 ...
                 st.info("Sélectionnez fournisseur(s).")
        elif len(semaine_columns) < 104:
            st.warning("Données historiques insuffisantes (< 104 sem).")
        else:
            st.markdown("#### Paramètres")
            ...
            if st.button("▶️ Lancer Simulation", key="run_fcst_sim"):
                 if not sel_months_fcst:
                    ...
                    with st.spinner("Simulation..."):
                        df_fcst_res = calculer_forecast_simulation(df_display_filtered, semaine_columns, sel_months_fcst, sim_t, prog_use, obj_use)
            ...
            if 'forecast_result_df' in st.session_state and st.session_state.forecast_result_df is not None:
                 ...
                 if st.session_state.get('forecast_params') == current_params_disp:
                    ...
                    fcst_disp_fin = [c for c in fcst_disp_cols if c in df_fcst_disp.columns]
