...
# --- Existing Helper Functions (safe_read_excel, format_excel_sheet, etc.) ---
...
def ai_calculate_order_quantities(df_products_for_ai, historical_semaine_cols, num_forecast_weeks,
    ...
    for i, (prod_idx, prod_row) in enumerate(df_calc_ai.iterrows()):
        ...
        if prod_ts_df_fit['y'].notna().sum() < 12: # Min data points
            ...
            st_prog_bar_ai.progress((i + 1) / num_prods)
            continue
        try:
            ...
            df_calc_ai.loc[prod_idx, "Qté Cmdée (IA)"] = 0; df_calc_ai.loc[prod_idx, "Forecast Ventes (IA)"] = 0.0
        st_prog_bar_ai.progress((i + 1) / num_prods)
    st_prog_bar_ai.empty() # Clear progress bar
    ...
    if min_order_amount_for_subset > 0 and current_total_amount_ia < min_order_amount_for_subset:
        ...
        if not eligible_inc_indices:
            ...
            # Get series for direct modification (faster than .loc for many updates)
            qtes_cmdees_ia_series_adj = df_calc_ai["Qté Cmdée (IA)"].copy() # Work on a copy
            ...
            while current_total_amount_ia < min_order_amount_for_subset and current_adj_iter < max_adj_iter:
                ...
                qtes_cmdees_ia_series_adj.loc[df_item_idx_inc] += pkg_adj # Update the series
                current_total_amount_ia += (pkg_adj * price_adj)
                item_ptr_adj = (item_ptr_adj + 1) % len(eligible_inc_indices)
            
            df_calc_ai["Qté Cmdée (IA)"] = qtes_cmdees_ia_series_adj # Assign back modified series
...
if 'df_initial_filtered' in st.session_state and isinstance(st.session_state.df_initial_filtered, pd.DataFrame):
    ...
    with tab1_ai:
        ...
        if not PROPHET_AVAILABLE:
            ...
            if sel_f_t1_ai:
                if not df_base_tabs.empty: df_disp_t1_ai = df_base_tabs[df_base_tabs["Fournisseur"].isin(sel_f_t1_ai)].copy(); st.caption(f"{len(df_disp_t1_ai)} art. / {len(sel_f_t1_ai)} fourn.")
            else: st.info("Sélectionner fournisseur(s).")
            st.markdown("---")

            if df_disp_t1_ai.empty and sel_f_t1_ai: st.warning("Aucun article pour fournisseur(s) sélectionné(s).")
            elif not id_sem_cols and not df_disp_t1_ai.empty: st.warning("Colonnes ventes historiques non identifiées. Prévision IA impossible.")
                ...
                with c2_ai:
                    ...
                    if len(sel_f_t1_ai) == 1 and sel_f_t1_ai[0] in min_o_amts and min_amt_ai_t1_default == 0.0: # Prefill for single supplier
                        min_amt_ai_t1_default = min_o_amts[sel_f_t1_ai[0]]
                    min_amt_ai_t1 = st.number_input("💶 Montant min total (€) (si 1 fourn.):", 0.0, value=min_amt_ai_t1_default, step=50.0, format="%.2f", key="min_amt_ai_t1_numin")
                
                # Store UI inputs to session state for persistence across reruns if calculation isn't triggered
                ...
                if st.button("🚀 Calculer Qtés avec IA", key="calc_q_ai_b_t1_go"):
                    curr_calc_params_t1_ai = {'suppliers': sel_f_t1_ai, 'forecast_weeks': fcst_w_ai_t1, 'min_amount_ui': min_amt_ai_t1, 'sem_cols_hash': hash(tuple(id_sem_cols))}
                    ...
                    for sup_idx_ai, sup_name_proc_ai in enumerate(sel_f_t1_ai):
                        ...
                        sup_specific_min_order_ai = 0.0
                        if len(sel_f_t1_ai) == 1: sup_specific_min_order_ai = min_amt_ai_t1 # Use UI global if single
                        else: sup_specific_min_order_ai = min_o_amts.get(sup_name_proc_ai, 0.0) # Use configured per-supplier
                    ...
                    if calc_ok_overall_ai and res_dfs_list_ai_calc:
                        ...
                        st.rerun()
                    elif not res_dfs_list_ai_calc: st.error("❌ Aucun résultat IA généré.")
                ...
                if 'ai_commande_result_df' in st.session_state and st.session_state.ai_commande_result_df is not None:
                    curr_ui_params_t1_ai_disp = {'suppliers': sel_f_t1_ai, 'forecast_weeks': fcst_w_ai_t1, 'min_amount_ui': min_amt_ai_t1, 'sem_cols_hash': hash(tuple(id_sem_cols))}
                    if st.session_state.get('ai_commande_params_calculated_for') == curr_ui_params_t1_ai_disp:
                        st.markdown("---");st.markdown("#### Résultats Prévision Commande (IA)")
                        df_disp_ai_res_final = st.session_state.ai_commande_result_df; total_amt_ai_res_final = st.session_state.ai_commande_total_amount
                        st.metric(label="💰 Montant Total Cmd Prévu (IA agrégé)", value=f"{total_amt_ai_res_final:,.2f} €")
                        
                        for sup_chk_min_ai in sel_f_t1_ai: # Check individual supplier minimums if they were applied
                            sup_min_cfg_val_ai = min_o_amts.get(sup_chk_min_ai, 0.0)
                            min_applied_in_calc_ai = 0.0
                            if len(sel_f_t1_ai) == 1: min_applied_in_calc_ai = min_amt_ai_t1 # This was the one used
                            else: min_applied_in_calc_ai = sup_min_cfg_val_ai # This was the one used

                            if min_applied_in_calc_ai > 0: # Only if a min was actually applied for this supplier
                        ...
                        st.markdown("#### Export Commandes (IA)")
                        ...
                        if not df_exp_ai_final_dl.empty:
                            ...
                            try:
                                with pd.ExcelWriter(out_b_ai_exp_dl, engine="openpyxl") as writer_ai_exp_dl:
                                    ...
                                    for sup_e_ai_dl in sel_f_t1_ai:
                                ...
                                if shts_ai_exp_dl > 0:
                                    ...
                                    fn_ai_dl=f"commandes_IA_{'multi'if len(sel_f_t1_ai)>1 else sanitize_sheet_name(sel_f_t1_ai[0])}_{pd.Timestamp.now():%Y%m%d_%H%M}.xlsx"
                                    st.download_button(f"📥 Télécharger ({shts_ai_exp_dl} feuilles)",out_b_ai_exp_dl,fn_ai_dl,"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",key="dl_ai_cmd_b_t1_dl")
                                else:st.info("Aucune qté IA > 0 à exporter (ou err création feuilles).")
                        ...
                        else:st.info("Aucun article qté IA > 0 à exporter.")
