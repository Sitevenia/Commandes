...
def calculer_quantite_a_commander(df, semaine_columns, montant_minimum, duree_semaines):
    try:
        ...
        st.error(f"Erreur dans le calcul des quantités à commander pour le fournisseur: {e}")
        return None
    ...
    try:
        ...
        if not df_filtered.empty:
            ...
            if result is not None:
                df_filtered["Quantité à commander"], df_filtered["Ventes N-1"], df_filtered["Ventes 12 semaines identiques N-1"], df_filtered["Ventes 12 dernières semaines"], montant_total = result
                ...
                st.error("Erreur lors du calcul des quantités à commander.")
        else:
